...
> 主窗口宠物面板 **含 3D 预览**，与独立宠物窗口(F17)不同。
> 独立宠物窗口是全功能 3D 宠物（透明无边框置顶），主窗口面板是控制/预览/设置一体。

### 包含内容

| Tab | 内容 |
|-----|------|
| 控制面板 Tab | 3D 预览 + 开关 + 模型信息 + 实时状态 |
...
├── PetControlTab.tsx      # 控制面板 Tab（3D 预览 + 开关 + 模型信息 + 实时状态）
├── Pet3DPreview.tsx       # 3D 预览（小型 Three.js 渲染区）
