...
vi.mock('@mmd/loaders/MMDLoader.js', () => {
  class MockMMDLoader {
...
vi.mock('@mmd/animation/MMDAnimationHelper.js', () => {
  return {
...
describe('MMD 渲染链路测试', () => {
  ...
  describe('1. 模块导入验证', () => {
    it('MMDLoader 可以从 three examples 模块导入', async () => {
      const { MMDLoader } = await import('@mmd/loaders/MMDLoader.js')
    ...
    it('MMDAnimationHelper 可以从 three examples 模块导入', async () => {
      const { MMDAnimationHelper } = await import(
        '@mmd/animation/MMDAnimationHelper.js'
      )
