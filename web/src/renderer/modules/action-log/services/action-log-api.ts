/**
 * 操作日志 API 服务
 * 后端 Query: page, page_size, module, action, start_time, end_time
 */

import { apiClient, extractData } from '@/services/api-client'
import type { ActionLog } from '../types/action-log'

export interface ActionLogListParams { page?: number; pageSize?: number; module?: string; action?: string; startTime?: string; endTime?: string }
export interface ActionLogListResponse { data: ActionLog[]; total: number; page: number; pageSize: number }
export interface CreateActionLogParams { module: string; action: string; paramsSummary: string; sessionId: string }
