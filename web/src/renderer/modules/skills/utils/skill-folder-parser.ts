...
const ruleCredentialTheft: ScanRule = {
  ...
  scan: (c, f) => findMatches(c, /(cat|read|load|open|ls|cp|mv|rm|curl|wget|source|\.)\s+(~\/\.ssh|~\/\.aws|~\/\.config|~\/\.gnupg|~\/\.docker)|\/etc\/(shadow|passwd)[^a-zA-Z]|\.env\b.*(?:read|cat|load|source)|credentials?\s*file/gi),
}
...
const rulePersistence: ScanRule = {
  ...
  // 要求完整命令格式，排除英文文档中自然提到这些词
  scan: (c, f) => findMatches(c, /crontab\s+(-[elru]|--)|systemctl\s+(enable|daemon-reload|start)\s+|\/etc\/init\.d\/[a-z]|echo\s+[^|]*>>\s*~?\/.*\.(bashrc|zshrc|profile)|launchctl\s+(load|write|start)\s+|schtasks\s+\/(create|change)\s+|\/Library\/Launch(Daemons|Agents)\/|systemd\s+daemon-reload|>>\s*rc\.local/gi),
}
...
const rulePrivilegeEscalation: ScanRule = {
  ...
  // 要求命令格式（sudo + 命令动词），排除自然语言中的 "use sudo to..."
  scan: (c, f) => findMatches(c, /\bsudo\s+(apt|yum|dnf|pip|npm|brew|rm|cp|mv|chmod|chown|mount|umount|systemctl|service|useradd|usermod|passwd|visudo|cat|echo|tee|bash|sh|python|node)|\bchmod\s+[0-7]{3,4}\s+\b|\bchown\s+[a-z]+:[a-z]+\s+\b|\bsetuid\b|\bsetgid\b|\/proc\/self\/exe/gi),
}
