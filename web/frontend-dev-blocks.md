...
| F30 | 灵魂系统 | ✅ (mock) | ❌ | ⭐⭐ | 1d |
| F31 | 天气 & 主动问候 | ✅ (mock) | ❌ | ⭐⭐ | 1d |
| F32 | 意图学习 & 行为模式检测 | ✅ (mock) | ❌ | ⭐⭐⭐ | 1.5d |

**总计：32 个开发块，约 36.5 个工作日**
...
| 借鉴点 | 说明 | 落地方案 |
|--------|------|---------|
| 嘴型同步 | 如果做语音对话，需要嘴型同步动画 | 可选功能，后期加 |
| 动作队列状态 | 显示当前动作队列播放状态 | 加到属性弹窗里 |
| 模型热重载 | 不重启窗口即可切换模型 | 加"重载模型"功能 |
...
### 🔍 竞品参考与借鉴（Live2D 桌面宠物控制面板）

> 参考产品：Live2D Cubism 5 桌面宠物控制面板
> 对比分析日期：2026-05-27
...
| 模块 | 功能 |
|------|------|
| 顶部导航 | 控制面板 / 模型管理 / 设置（3个Tab） |
| 左侧 | Live2D 模型实时预览区 |
| 右侧 | 桌面宠物开关 + 模型信息 + 实时状态 |
| 操作按钮 | 关闭宠物窗口 / 刷新 / 重载模型 |
| 模型信息 | 模型名/版本/表情数/动作数/鼠标跟随 |
| 实时状态 | 库加载/鼠标跟随/点击交互/嘴型同步/当前动作/表情/气泡/动作队列 |
| 帧率控制 | 30FPS / 120FPS / 无限制（手动选择） |

#### ✅ 我们已有的优势

| 功能 | 参考产品 | Beautiful-Elf | 评价 |
|------|---------|---------------|------|
| 六维属性系统 | ❌ 没有 | ✅ 饥饿/清洁/心情/健康/亲密/等级 | 我们更强 |
| 互动系统 | ❌ 没有 | ✅ 喂食/清洁/聊天 + 互动记录 | 我们更强 |
| 属性衰减 | ❌ 没有 | ✅ 每小时自动衰减 + 离线结算 | 我们更强 |
| AI 气泡对话 | ❌ 没有 | ✅ Ollama 生成自然语言 | 我们更强 |
| 亲密度&等级 | ❌ 没有 | ✅ 成长系统，解锁新表情/动作 | 我们更强 |
| 帧率自适应 | 手动选 30/120/无限制 | 自动 5fps/60fps | 我们更智能 |
| 通知联动 | ❌ 没有 | ✅ 宠物状态变化触发通知 | 我们更强 |
| 游戏性 | 仅展示模型 | 养成玩法 | 我们更有温度 |

#### 💡 借鉴点（建议加入）

| 优先级 | 借鉴点 | 说明 | 落地方案 |
|--------|--------|------|---------|
| ⭐⭐⭐ | **操作按钮组** | 关闭/刷新/重载 三个按钮并排，操作方便 | F18 控制面板 Tab 顶部加按钮组 |
| ⭐⭐⭐ | **状态指示灯** | 绿色圆点表示运行状态，一目了然 | F18 实时状态用绿色圆点 |
| ⭐⭐ | **模型管理 Tab** | 独立的模型导入/切换/管理功能 | F18 设置 Tab 增加模型管理区，或新增 Tab |
| ⭐⭐ | **帧率手动控制** | 让用户自己选 30/120/无限制 | F18 设置 Tab 加帧率选项（高级选项） |
| ⭐⭐ | **实时模型预览** | Live2D 实时渲染预览（比截图更直观） | F18 可选：省资源模式用截图，高性能模式用小尺寸 Three.js 渲染 |
| ⭐ | **嘴型同步状态** | 显示嘴型同步是否开启 | 如果做语音对话功能，加到实时状态 |
...
│                                             │
│  ┌─ 桌面宠物 ─────────────────────────────┐  │
│  │ 开启后将在桌面显示 Live2D 宠物    [开关] │  │
│  └────────────────────────────────────────┘  │
│                                             │
│  ┌─ 模型信息 ─────────────────────────────┐  │
│  │ 模型名称: Haru          版本: Cubism 5  │  │
│  │ 表情数: 8              动作数: 2        │  │
│  │ 鼠标跟随: ● 开启                        │  │
│  └────────────────────────────────────────┘  │
│                                             │
...
```
参考产品 = Live2D 模型展示工具（专业但"冷"）
我们的产品 = AI 桌面宠物养成（有"温度"）

它强在：Live2D 专业控制、实时预览、模型管理
我们强在：养成系统、AI 互动、游戏性、情感连接

借鉴方向：把它的"专业控制"能力吸收过来，加上我们的"养成玩法"优势
```
...
> dev-doc 3.5 灵魂系统：每个助手拥有独立人格，首次引导创建，运行时注入提示词。

### 包含内容

| 功能 | 说明 |
|------|------|
| 首次引导 | Ant Design Steps 引导用户创建专属助手人格 |
| 人格配置 | 名称、头像、性格标签、说话风格、情感倾向 |
| 人格预览 | 预览助手人格效果 |
| 人格切换 | 支持多个人格配置切换 |
| 运行时注入 | 对话时注入人格系统提示词 |
...
```
soul/
├── SoulGuide.tsx          # 首次引导（Steps 组件）
├── SoulForm.tsx           # 人格配置表单
├── SoulPreview.tsx        # 人格预览
├── SoulList.tsx           # 人格列表
└── SoulSwitcher.tsx       # 人格切换器
```
...
interface SoulConfig {
  ...
  personality: string[];  // 性格标签（温柔/活泼/严肃...）
...
> dev-doc 4.5 天气服务 + 主动问候：查询天气 + AI 生成个性化问候语。
...
```
weather/
├── WeatherWidget.tsx      # 天气小组件（可放在首页）
├── WeatherCard.tsx        # 天气卡片
├── WeatherForecast.tsx    # 未来 3 天预报
├── GreetingBubble.tsx     # 主动问候气泡
└── weather-service.ts     # 天气 API 调用
```
...
const greetings = {
  morning: {
    rain: "早上好！今天有雨，记得带伞 🌧️",
    sunny: "早安！今天天气不错，适合出门 ☀️",
    cold: "早上好！今天有点冷，多穿点 🧣",
  },
  afternoon: "下午好！工作辛苦了，喝杯咖啡吧 ☕",
  evening: "晚上好！今天过得怎么样？🌙",
};
```
...
> dev-doc 6.1 意图学习系统 + 6.2 行为模式检测与技能自动建议。
...
```
intent-learning/
├── IntentCorrection.tsx   # 意图纠正弹窗
├── CorrectionHistory.tsx  # 纠正历史记录
├── PatternDisplay.tsx     # 行为模式展示
├── SkillSuggestion.tsx    # 技能建议通知
└── SuggestionActions.tsx  # 建议操作按钮
```
...
> dev-doc 8.3 自定义 Hooks，应作为 F01 的一部分。
...
| `useDebounce` | 搜索防抖 |
| `useWebSocket` | WebSocket 连接管理、心跳检测与重连 |

---

## 📦 扩展：F04 补充 - 轮询管理 & 操作撤销

> dev-doc 7.2 轮询管理 + 7.5 操作撤销，应作为 F04 的一部分。

### 轮询管理 PollingRegistry
...
function showUndoToast(message: string, onUndo: () => void) {
  ...
  message.info({
    content: `${message}`,
    btn: (
      <Button type="link" size="small" onClick={() => { onUndo(); message.destroy(key); }}>
        撤销
      </Button>
    ),
    key,
    duration: 30,  // 30 秒后自动消失
  });
}
```
...
> dev-doc 7.5 桌面通知，应作为 F20 的一部分。

```typescript
// Electron 原生 Notification API
...
## 📦 扩展：F21 补充 - Prompt 版本管理

> dev-doc 3.1 Prompt 版本管理，应作为 F21 的一部分。

| 功能 | 说明 |
|------|------|
| Prompt 列表 | 展示所有 Prompt（名称、版本、是否激活） |
| 版本历史 | 查看历史版本，支持回滚 |
| 版本对比 | 两个版本的 diff 对比 |
| A/B 测试 | 配置多个版本按比例分配 |
...
> dev-doc 6.6 视觉模块，比单纯的截图选区更完整。

### 扩展内容

| 功能 | 说明 |
|------|------|
| 屏幕捕获 | Electron desktopCapturer 获取视频流 |
| 选区捕获 | Canvas overlay 画矩形选区 |
| 帧变化检测 | pHash 算法，每帧计算哈希值比较差异 |
| 变化阈值 | 默认 5%，可配置 1%～20% |
| 场景自适应 | 代码编辑 3% / 视频游戏 15% |
| AI 分析模式 | general（描述）/ text（OCR）/ code（代码识别） |
| 主动推送 | 检测到特定内容时提醒 |
...
├── SelectionOverlay.tsx   # 选区 Canvas overlay
├── FrameDetector.tsx      # 帧变化检测
├── AiAnalysis.tsx         # AI 分析
├── AnalysisMode.tsx       # 分析模式切换
├── ThresholdSettings.tsx  # 阈值设置
└── VisualAlert.tsx        # 主动推送提醒
```
...
```
第一批（基础框架，3天）
  F01 脚手架(+Hooks) → F02 布局 → F03 主题 → F04 状态管理(+轮询+撤销)
...
第五批（独立模块，5天）
  F17 3D 宠物窗口 → F18 宠物面板 → F19 性能监控 → F20 通知中心(+桌面通知)

第六批（系统功能，5.5天）
  F21 设置(+Prompt版本) → F22 文件预览 → F23 OCR → F24 视觉模块(扩展)

第七批（智能化，3.5天）
  F30 灵魂系统 → F31 天气问候 → F32 意图学习
...
| 模块 | 状态 | 说明 |
|------|------|------|
| F01 项目脚手架 | ✅ | 完整，补充了自定义 Hooks |
| F02 全局布局 | ✅ | 完整 |
| F03 主题系统 | ✅ | 完整 |
| F04 状态管理 | ✅ | 完整，补充了轮询管理和撤销 |
| F05 命令面板 | ✅ | 完整 |
| F06 聊天面板 | ✅ | 完整（含意图识别、推理深度、语义缓存） |
| F07 日程模块 | ✅ | 完整 |
| F08 剪贴板模块 | ✅ | 完整 |
| F09 代码片段 | ✅ | 完整 |
| F10 知识库 | ✅ | 完整 |
...
| F16 工具管理 | ✅ | 完整 |
| F17 3D 宠物 | ✅ | 完整 |
| F18 宠物面板 | ✅ | 已修正为截图预览，复杂度提升 |
| F19 性能监控 | ✅ | 完整 |
| F20 通知中心 | ✅ | 完整，补充了桌面通知 |
| F21 设置系统 | ✅ | 完整，补充了 Prompt 版本管理 |
| F22 文件预览 | ✅ | 完整 |
| F23 OCR | ✅ | 完整 |
| F24 视觉模块 | ✅ | 已扩展为完整视觉模块 |
| F25 托盘快捷键 | ✅ | 完整 |
