...
function removePassword(input) {
    ...
    input = input.replace(flagPattern, (_match, prefix, quote, quotedVal, unquotedVal) => {
        ...
        const hashed = (0, crypto_1.createHash)("sha256").update(value).digest("hex");
        return `${prefix} ${quote !== null && quote !== void 0 ? quote : ""}${hashed} (sha256 hash)${quote !== null && quote !== void 0 ? quote : ""}`;
    });
    ...
    return input.replace(/(\/b\s+)(.*?)(\s+\/c)/g, (_match, p1, p2, p3) => {
        const hashed = (0, crypto_1.createHash)("sha256").update(p2).digest("hex");
...
function filterSensitiveEnv(env) {
    ...
    for (const [k, v] of Object.entries(env)) {
        out[k] = SENSITIVE_ENV_KEY_RE.test(k) && v != null ? `${(0, crypto_1.createHash)("sha256").update(v).digest("hex")} (sha256 hash)` : v;
    }
    return out;
}
...
async function executeAppBuilder(args, childProcessConsumer, extraOptions = {}, maxRetries = 0) {
    ...
    if (maxRetries === 0) {
        ...
        return (0, builder_util_runtime_1.retry)(runCommand, { retries: maxRetries, interval: 1000 });
    }
}
