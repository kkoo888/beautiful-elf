...
async function addLicenseToDmg(packager, dmgPath) {
    ...
    for (const button of licenseButtonFiles) {
        ...
        const label = filepath.endsWith(".yml") ? (0, js_yaml_1.load)(await (0, fs_extra_1.readFile)(filepath, "utf-8"), { schema: js_yaml_1.CORE_SCHEMA }) : await (0, fs_extra_1.readJson)(filepath);
        if (label.description) {
            // to support original button file format
            label.message = label.description;
            delete label.description;
        }
        jsonFile.labels.push((0, builder_util_1.deepAssign)({ lang: button.langWithRegion.replace("_", "-") }, label));
    }
