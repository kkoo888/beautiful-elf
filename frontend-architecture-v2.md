...
| 🔔 通知 | 通知历史中心（见下方详细说明） |
| 🐾 宠物 | 仅属性展示和控制（见下方详细说明） |
| ⚙️ 设置 | Ollama / AI / 应用 / 快捷键 / 隐私安全 / 关于更新 |
...
> 主窗口宠物面板**不做 3D 渲染**，3D 预览完全留给独立宠物窗口。

| Tab | 内容 |
|-----|------|
| 状态 Tab | 六维属性 Progress 实时展示 + 当前状态文字/动画名称 + 模型信息 (名称/路径) |
| 互动 Tab | 喂食 / 清洁 / 聊天 操作按钮 + 互动记录列表 |
| 设置 Tab | 模型路径 / 窗口配置 / 衰减速度 / 气泡频率 + **[打开宠物窗口] 按钮** |
