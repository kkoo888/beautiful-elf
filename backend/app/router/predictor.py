...
from app.router.features import ContextMetadata
from app.router.flags import RoutingFlags, compute_flags
...
class SquillaRouter:
    ...
    def __new__(cls, model_dir: str = "models/", *args, **kwargs):
        ...
        if version_path.exists():
            try:
                ...
                ver = None
            if ver == "v4":
                from app.router.v4_predictor import V4Router
    ...
    def __init__(self, model_dir: str = "models/",
        ...
        from app.router.features import FeatureExtractor
    ...
    def predict(self, text: str, context: ContextMetadata | None = None, *,
        ...
        from app.router.trajectory import classify as classify_trajectory
...
# ── 便捷接口（供 ModelSelector 调用）────────────────────

def load_config(model_dir):
    """加载 router.runtime.yaml 配置"""
    import yaml as _yaml
    config_path = model_dir / "router.runtime.yaml"
...
class RouterPredictor:
    """路由器封装类（供 ModelSelector 使用）"""

    def __init__(self, model_dir):
        self._model_dir = model_dir
        self._config = load_config(model_dir)
        self._router = None
        self._init_router()

    def _init_router(self):
        try:
            self._router = SquillaRouter(
                model_dir=str(self._model_dir),
                config_path=str(self._model_dir / "router.runtime.yaml"),
            )
        except Exception as e:
            import logging
            logging.getLogger(__name__).warning(f"[RouterPredictor] 初始化失败: {e}")
...
def predict_route(text, predictor, config=None, history=None):
    """便捷推理函数"""
    if not predictor.available:
        raise RuntimeError("RouterPredictor 不可用")
    result = predictor._router.predict(text, history=history)
    flags_dict = {}
    if hasattr(result.flags, '__dataclass_fields__'):
        flags_dict = {f: getattr(result.flags, f, False) for f in result.flags.__dataclass_fields__}
    return {
