...
async def get_active_prompt(
    ...
    result = await _service.get_active(db, name)
...
async def get_prompt_versions(
    ...
    return success(await _service.get_versions(db, name))


@router.get("/{prompt_id}")
async def get_prompt(prompt_id: int, db: AsyncSession = Depends(get_db)):
    return success(await _service.get_by_id(db, prompt_id))
...
async def update_prompt(
    ...
    return success(await _service.update(db, prompt_id, data))
...
async def activate_prompt(prompt_id: int, db: AsyncSession = Depends(get_db)):
    return success(await _service.activate(db, prompt_id))
