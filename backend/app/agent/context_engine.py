...
_PROMPT_TASK_COMPLETION = (
    ...
    "工具失败就直接说明，尝试替代方案。绝对不要编造数据替代无法产出的结果。"
)

# ── 记忆指导 ─────────────────────────────────────────────
# 解决的问题：记忆写入质量差（保存临时状态、用祈使句写记忆）
_PROMPT_MEMORY_GUIDANCE = (
    "\n\n## Memory guidance（记忆使用指导）\n"
    "用记忆工具保存持久事实：用户偏好、环境信息、稳定约定。保持紧凑。\n"
    "不要保存任务进度、会话结果、临时 TODO。\n"
    "写记忆用陈述性事实：「用户偏好简洁回答」✓ — 「始终简洁回答」✗"
)
