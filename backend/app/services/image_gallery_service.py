...
class ImageGalleryService:
    ...
    async def generate_prompt(self, db: AsyncSession, data: PolishPromptRequest) -> str:
        ...
        polish_system = (
            ...
            "- 200-400 字为宜\n\n"
            "【人物姿态核心原则 — 涉及人物时必须遵守】\n"
            "1. 参考杂志封面、时尚大片、电影剧照的拍摄风格\n"
            "2. 人物必须有自然的动作和表情，避免僵硬摆拍感\n"
            "3. 视线方向随机化——侧目、回眸、低头沉思、望向远方、仰视天空等，不要总是直视镜头\n"
            "4. 肢体要有动态感：微微侧身、手部自然摆放、头发飘动、衣袂轻扬\n"
            "5. 表情要有情绪：浅笑、慵懒、若有所思、自信从容、神秘冷艳\n"
            "6. 参考关键词：杂志封面、时尚摄影、电影感、自然抓拍、动态瞬间\n\n"
    ...
    async def describe_image(self, db: AsyncSession, data) -> str:
        ...
        message = HumanMessage(content=[
            {"type": "text", "text": (
                ...
                "- 描述要具体、生动、有画面感\n"
                "- 人物姿态参考杂志封面、时尚大片风格，避免僵硬摆拍\n"
                "- 视线方向随机化（侧目、回眸、望向远方等），不要总是直视镜头\n"
                "- 200-400 字为宜\n"
                "- 直接返回提示词正文，不要任何标题或前缀"
            )},
